...
class PurchaseAdvancePayment(models.TransientModel):
		...
		if company.currency_id == currency:
			if self.advance_amount == 0.0:
				...
				if self.multi_currency_id:
					total += self.multi_currency_id.compute(self.advance_amount, company.currency_id)
					self.multi_currency_id = False
				else:
					total += company.currency_id.compute(self.advance_amount, currency)
		else:
			total += company.currency_id.compute(self.advance_amount, currency)
	...
	def gen_purchase_advance_payment(self):
		...
		if self.currency_id != self.company_id.currency_id:
			...
			advance_amount = self.currency_id.compute(self.advance_amount, self.company_id.currency_id)
		...
		purchase_obj = self.env['purchase.order'].browse(self._context.get('active_ids', []))
		advance_lines = []
		if self.advance_amount > 0:
		
			currency_id = self.company_id.currency_id
			for advance in purchase_obj:
				advance_lines.append((0,0,{
					'name': self.name,
				...
				advance.update({
					'payment_history_ids': advance_lines,	
				})
...
class AdvancePaymentHistoryPurchase(models.Model):
	_name = 'payment.history.purchase'
